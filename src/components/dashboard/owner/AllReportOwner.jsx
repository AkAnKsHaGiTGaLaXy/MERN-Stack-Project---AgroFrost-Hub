  ...
  const fetchColdStorageProfile = async () => {
    ...
    try {
      const response = await axios.get(
        `${endpoint}/owner/get/storage/${user?.uid}`,
        config
      );
      if (response?.data != null) {
        setQueryData((prevData) => ({
          ...prevData,
          csId: response?.data?.cs_id,
        }));
        setQueryMonth((prevData) => ({
          ...prevData,
          csId: response?.data?.cs_id,
        }));
    ...
    try {
      ...
      if (option === "weekly") {
        response = await axios.get(
          `${endpoint}/owner/reports/weekly/${ColdStorageData?.cs_id}`,
          config
        );
      } else if (option === "monthly") {
        response = await axios.get(
          `${endpoint}/owner/reports/monthly/${ColdStorageData?.cs_id}`,
          config
        );
      } else if (option === "yearly") {
        response = await axios.get(
          `${endpoint}/owner/reports/yearly/${ColdStorageData?.cs_id}`,
          config
        );
      }
    ...
    return (
      <div className="flex justify-between items-center mb-3 border-b">
        <div>
          ...
          <p>Cold Storage Id : {ColdStorageData.cs_id}</p>
          <p>Cold Storage Name: {ColdStorageData.cs_name}</p>
          <p>Address : {ColdStorageData.cs_address}</p>
        </div>

        <div>
          <h3 className="font-bold">Cold Storage Owner Details</h3>
          <p>Owner Id : {ColdStorageData.owner_id}</p>
          <p>Owner Name : {ColdStorageData.owner_ref.owner_fullName}</p>
          <p>Owner Email: {ColdStorageData.owner_ref.owner_email}</p>
          <p>Owner Contact No : {ColdStorageData.owner_ref.owner_contactNo}</p>
        </div>
      </div>
    );
  };
